...
class TestAuthTokens:
    ...
    async def test_error_when_token_or_secret_is_wrong(
        ...
        with pytest.raises(HTTPException):
            auth_service.get_username_from_token(
...
class TestUserLogin:
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            # ("email", None),  # in current implementation it cannot be None - leads to 422 status code
            ("email", "notemail"),
            ("password", "wrongpassword"),
            # ("password", None),  # in current implementation it cannot be None - leads to 422 status code
        ),
    )
